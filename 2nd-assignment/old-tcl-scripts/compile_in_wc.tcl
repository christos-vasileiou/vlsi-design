...
echo "################## Start synthesis for HOLD-time violation ##################\n\n\n"
set target_library sgc25a.min.db 
set symbol_library sgc25a.sdb
set link_library [concat  "*" $target_library]

# /* add I/O library: */
set target_library [concat  $target_library sgc25a_io.min.db ]
set symbol_library [format "%s%s"  $symbol_library sgc25a_io.sdb]
set link_library [concat  "*" $target_library]
...
write_sdf netlist/mips.sdf
write -format verilog -h -o netlist/mips.v
report_timing -transition_time -delay_type max -significant_digits 10 > reports/report_timing.txt
report_area -designware -physical > reports/report_area.txt
report_design -physical > reports/report_design.txt
